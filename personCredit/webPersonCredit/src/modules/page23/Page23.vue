...
<div class="pagePersonal">
     <mt-header class="mint-header is-fixed" title="简历辨伪">
...
<p class="topHint">目前只支持前程无忧、猎聘网的简历模板上传</p>



              <div class="personal-form ">
           <div class="list-box">
                         <div class="list-input extra-input">
                            <p class=" "> <span class="fcRed f14">* &nbsp; </span>查询身份证号码</p>
                            <input v-model="code.id" placeholder="请填写">
                        </div>
...
// if (!isIDCard2.test(this.code.id)) {
//   let instance = Toast('请重新核对身份证号码');
//   setTimeout(() => {
//   instance.close();
//   }, 1500);
//   return false;
...
this.$http.post('/personCredit/companyAdmin/resumeAnalysis.do',formData)
.then((res)=>{
Indicator.close()
if(res.body.status=='ok'){
// console.log(res.body.list)

var list=JSON.parse(JSON.stringify(res.body.list));
...
this.upLoad=true; 
      ...
      this.$store.commit("updateFactList",list)
      console.log(this.$store.state.factList)

      setTimeout(() => {
this.$router.push("page14")
}, 1000);
...
value(val) {
        if (val >= 100) {
          ...
          setTimeout(() => this.$router.push('page20'), 1200);
