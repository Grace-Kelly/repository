...
<!-- v-nav-title="page19" -->
<div class="page19">
<mt-header class="mint-header " v-if="!userLogin" title="个人注册">
</mt-header>
<mt-header class="mint-header " v-if="userLogin" title="个人登录">
</mt-header>
<main>
    <div class="mui-content ">
      <div class="mui-col-xs-12 mui-text-center">
          ...
          <div class="list-input">
              ...
              <input v-on:focus="vnFocus" v-on:blur="vnBlur" v-model="user.telPhone"  placeholder="请输入手机号">
          </div>
          <div class="list-input">
              <p class="iconfont icon-mima"></p>
              <input type="password" v-on:focus="vnFocus" v-on:blur="vnBlur" v-model="user.userPwd" placeholder="请设置密码">
          </div>
          <div class="list-input">
              <p class="iconfont icon-mima"></p>
          ...
          <div class="list-input">
              <p class="iconfont icon-iconfont"></p>
              <input v-on:focus="vnFocus" v-on:blur="vnBlur" v-model="user.telPhone"  placeholder="请输入身份证或手机号">
          </div>
          <div class="list-input">
              <p class="iconfont icon-mima"></p>
              <input type="password" id="password" v-on:focus="vnFocus" v-on:blur="vnBlur" v-model="user.userPwd" placeholder="请输入密码">
...
<footer v-if="footer">
    ...
    <div class="switching" @click='gotoPage("page1")'>
      切换企业
    </div>
</footer>
    ...
    methods: {
     ...
     let isPassword= /^[A-Za-z0-9]{6,16}$/;
        if (!this.user.telPhone) {
         ...
         if (((phoneNumber.test(this.user.telPhone)||isIDCard1.test(this.user.telPhone)||isIDCard2.test(this.user.telPhone))===false)  ) {
           console.log((!(phoneNumber.test(this.user.telPhone)||isIDCard1.test(this.user.telPhone)||isIDCard2.test(this.user.telPhone))===false)  )
           console.log( phoneNumber.test(this.user.telPhone) )
           console.log( isIDCard2.test(this.user.telPhone) )
           console.log( isIDCard2.test(this.user.telPhone) )
                  let instance = Toast('手机号或身份证格式错误');
                    setTimeout(() => {
                      instance.close();
                    }, 1500);
                    return false;
         ...
         if (!isPassword.test(this.user.userPwd)) {
                   let instance = Toast('密码6-10位数字或字符串');
                    setTimeout(() => {
                      instance.close();
                    }, 1500);
                    return false;
...
Indicator.open();
       let user =new Object;
   user.telPhone=this.user.telPhone;
   user.userPwd=this.user.userPwd;
    let  loginUser = JSON.stringify(user);
      console.log(loginUser);
  this.$http.post('/personCredit/loginUser.do',loginUser)
                  .then((res)=>{
                    Indicator.close()
      ...
      logon(){
        ...
        setTimeout(()=>{ $(el).removeClass("btnActive")},200)
        //测试  
        //18588270177   140106198912130619
     let phoneNumber = /(^13\d{9}$)|(^14)[5,7]\d{8}$|(^15[0,1,2,3,5,6,7,8,9]\d{8}$)|(^17)[6,7,8]\d{8}$|(^18\d{9}$)/;
     let isIDCard1=/^(\d{6})()?(\d{2})(\d{2})(\d{2})(\d{2})(\w)$/;
     let isIDCard2=/^(\d{6})()?(\d{4})(\d{2})(\d{2})(\d{3})(\w)$/;
     let isPassword= /^[A-Za-z0-9]{6,16}$/;
        ...
        //             return false; 
        //   }
         if (!phoneNumber.test(this.user.telPhone)){
           console.log( isIDCard2.test(this.user.telPhone) )
           console.log( isIDCard2.test(this.user.telPhone) )
                  let instance = Toast('手机号格式错误');
                    setTimeout(() => {
                      instance.close();
                    }, 1500);
                    return false; 
         ...
         if (this.user.userRePwd!==this.user.userPwd) {
...
Indicator.open();
 ...
 var user = JSON.stringify(this.user);
              this.$http.post('/personCredit/registerUser.do',user)
                  .then((res)=>{
                    Indicator.close()
                    // if(res.body.status=='ok'){
                    //     MessageBox.confirm('前往添加认证','认证提示').then(
