...
public class PersonWorkServiceImpl implements PersonWorkService {
	...
	public Map<String, Object> savePersonWorkInfo(Map param) throws Exception {
			...
			if (!StringUtils.isEmpty(telPhone)) {
				...
				if (!StringUtils.isEmpty(saveFlag)) {
					if ("local".equals(saveFlag)) {
						personWorkDao.savePersonWorkInfo(param);
	...
	public List queryCompanyById(String memberId) throws Exception {
		return personWorkDao.queryCompanyById(memberId);
	}

	public List queryAllWorkById(String memberId) throws Exception {
		return personWorkDao.queryAllWorkById(memberId);
	}

	public Map<String, Object> updateWorkInfoBy(Map<String, Object> param) throws Exception {
		String saveFlag = (String) param.get("saveFlag");
		String telPhone = (String) param.get("telPhone");
		Map<String, Object> resultMap = new HashMap<String, Object>();
		if (!StringUtils.isEmpty(saveFlag)) {
			if ("local".equals(saveFlag)) {
				personWorkDao.updateWorkInfoById(param);// 更新
	...
	public String beanToJson(Map<String, Object> param) throws Exception {
		...
		if (param != null) {
			sendwork.setCompanName((String) param.get("companName"));
			sendwork.setDepartment((String) param.get("department"));
			sendwork.setDimissionTime(DateUtil.dateToString((Date) param.get("dimissionTime")));
			sendwork.setEntryTime(DateUtil.dateToString((Date) param.get("entryTime")));
			sendwork.setPosition((String) param.get("position"));
			sendwork.setTrade((String) param.get("trade"));
			sendwork.setWorkDescription((String) param.get("workDescription"));
			JSONObject obj = JSONObject.fromObject(sendwork);
			return obj.toString();
		} else {
